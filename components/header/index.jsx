    ...
    <header className={Styles.header}>
        ...
        <nav className={Styles.navbar}>
            ...
            <ul className={`${hamburger ? "flex":"hidden"} `}>
                ...
                <li onClick={() => {setActive(2); setHamburger(false)}} className={`${active == 2 && "text-[#61D0D4]"}`}><Link href="/blog" title='Blog'>BLOG</Link></li>
                <li onClick={() => {setActive(3); setHamburger(false)}} className={`${active == 3 && "text-[#61D0D4]"}`}><Link href="/projects" title='Projects'>PROJECTS</Link></li>
